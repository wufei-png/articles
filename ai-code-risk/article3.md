...
这篇文章想把问题再往上移一层。即使不看前文，也可以把它理解成一个独立的问题：当 AI Agent 不只参与写代码，也被用来优化团队指标，研发组织该如何避免陷入局部最优？

如果说前两篇讨论的是代码库，那么这篇讨论的是组织。更具体一点，是软件研发团队到底在奖励什么。
...
人会适应指标，团队会适应指标。agent 则会在这些指标塑造出来的任务里工作，把这种适应变得更便宜、更快、更连续。
...
这几类研究放在一起，提醒很简单：**生产力是系统性质，不是单点产出。**
...
如果组织只奖励“可见的完成”，这些工作会被挤出。大家未必不知道它们重要，只是指标里没有它们的位置。
...
组织层面不能简单套用这个说法。多数 coding agent 不会直接拿团队 KPI 做在线优化。

但指标会先改变人的选择：任务怎么拆，提示词怎么写，验收标准怎么定，什么内容能在周报和绩效里被看见。随后，agent 会沿着这些选择继续放大产出。

这不是说每个团队都会故意刷数据。更常见的是，指标慢慢改变了“什么值得做”的感觉。
...
传统自动化通常只能优化某个固定流程。Agent 更像一个通用执行器，它可以把很多组织意图翻译成行动。只要方向给错，它就能很勤奋地执行到底。
...
![团队目标与反向约束](./assets/article3/team-objective-guardrails.png)
...
好的指标会逼迫判断发生；坏的指标会把判断伪装成数字。

## 不要把 AI 使用率当成成熟度

接下来几年，很多团队会很自然地追踪这个数字。

多少工程师使用了 coding agent？多少 PR 有 AI 参与？多少测试是 AI 补的？多少文档由 AI 生成？多少任务进入了自动化流水线？

这些数据可以看，但要非常小心。

它最多说明工具被采用了，不说明团队变强了；它最容易制造的错觉，就是把“用了新工具”误当成“完成了组织升级”。
...
这并不要求团队回到低效率。AI 带来的执行速度越高，组织越需要在更上游保留慢判断。

所谓慢判断，指的是愿意花时间确认目标，愿意删除不该做的工作，愿意为了长期可维护性放慢一轮，愿意在指标很好看时仍然问一句：我们是不是正在优化错东西？
