...
很多工程问题，过去坏就坏在太依赖人的临场记忆。某个脚本为什么这么写，某个配置为什么不能改，某个服务为什么上线前要手动检查一项指标，知道的人可能只有两个，其中一个已经转组。
...
在结果可验证、失败可回滚、成本可承受的普通互联网服务里，这个模式很强。代码库向 agent 友好本身不是问题；真正的问题是：**当一个局部最优足够强，它会开始改造整个环境。**
...
代码风格被统一，本身未必是坏事。团队本来就会使用 formatter、lint、review 规范来消除无意义差异。真正值得警惕的，不是“每个人的代码看起来不一样”这件事消失了，而是更深一层的差异也可能被抹平：
...
真正需要保留的，不是表面的不一致，而是经过验证后仍然成立的设计选择空间：不同模块是否有清楚的失败假设，接口边界是否能随业务变化重新切分，验证手段是否不只依赖同一种测试，关键路径是否有可替换方案。
...
所以真正的问题不是“要不要一致”，而是“一致来自哪里”。如果所有团队都围绕类似的模型、类似的评测器、类似的工具接口和类似的最佳实践来组织软件，差异会从更底层开始减少。代码风格趋同只是最容易看见的一层。**更深的是接口趋同、验证方式趋同、任务拆分方式趋同、组织判断方式趋同。**
...
于是团队会自然地把更多任务改造成这种形状。需求写得更像 prompt，代码库写得更像 agent 的搜索空间，测试写得更像奖励函数，review 写得更像自动评估，工程师的工作变成设计约束和处理异常。
...
一些现实研究已经给了类似信号。DORA 的研究也提示，AI 采用可能会放大组织已有问题，交付稳定性和审查压力不一定自动改善。GitClear 对代码变更的观察则指出，AI 高采用环境下可能伴随重复代码增加、重构减少等质量信号。
...
但这不等于人类退出，而是位置上移。人类不应该把自己的价值理解成“继续亲手敲每一行代码”。**人类真正要守住的是：目标、边界、解释权、异常处置权和最终责任。**
...
可以有 AGENTS、CLAUDE 或类似说明文件，可以有明确的测试钩子、脚本入口、日志规范和自动化检查。但关键知识不要只写成某个模型、某个工具、某个厂商最顺手的格式。

真正好的 agent-friendly 代码库，应该同时也是 **human-friendly、tool-friendly、model-neutral** 的代码库。它可以被不同模型读取，可以被人类审查，可以在工具替换时继续工作。
...
这些系统不能把“模型能跑通”当成“系统可信”。它们需要独立评测、冗余机制、审计记录、可停用设计和明确的人类责任节点。NIST 等治理框架已经把生成式 AI 的同质化、人机配置和高风险部署问题放进风险视野里，本质也是在提醒：自动化越强，控制权设计越重要。

## 最后：世界会不会也在向模型收敛？

如果只看软件工程，这个问题已经足够大了。

但它还有一层更远的含义：软件并不只是代码。软件正在组织交易、内容分发、城市运行、企业管理、教育流程、医疗辅助和公共服务。代码库的形状，某种程度上也是世界运行方式的一部分。
...
但越是强大的工具，越会改造使用它的环境。真正值得警惕的，不是 AI 写了多少代码，也不是代码有没有保留工程师的个人风格，而是：**我们有没有意识到，自己正在把代码库、工作流、组织判断，甚至一部分世界运行方式，改造成 AI 更容易优化的形状。**
...
- [GitClear: AI Copilot Code Quality 2025](https://www.gitclear.com/ai_assistant_code_quality_2025_research)：关于重复代码、短期 churn 和 moved lines 下降等代码质量信号。
