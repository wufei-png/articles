...
代码风格被统一，本身未必是坏事。团队本来就会使用 formatter、lint、review 规范来消除无意义差异。要警惕的，不是“每个人的代码看起来不一样”这件事消失了，而是更深一层的差异也可能被抹平：
...
这些系统不能把“模型能跑通”当成“系统可信”。它们需要独立评测、冗余机制、审计记录、可停用设计和明确的人类责任节点。NIST AI 600-1 已经把同质化、人机配置、供应链和组件集成、可停用或快速终止能力等纳入生成式 AI 风险管理动作。放到 coding agent 场景里，重点是独立验证、审计和接管，而不是把“模型能跑通”当成系统可信。
